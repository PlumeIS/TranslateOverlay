...
public class  LocalOCR implements BaseOCR {
    ...
    public List<AnimationOCRResult> OCR(NativeImage screenshot) {
        try {
            ...
            if (posted == null) {
                ChatTranslator.sendBypassMessage(new TranslatableText("message.translateoverlay.ocr.request.failure", "远程错误"));
                return new ArrayList<>();
            }
            JsonObject jsonObject = posted.getAsJsonObject();
            if (!jsonObject.get("success").getAsBoolean()) {
                ChatTranslator.sendBypassMessage(new TranslatableText("message.translateoverlay.ocr.request.failure", jsonObject.get("message").getAsString()));
                return new ArrayList<>();
            }
