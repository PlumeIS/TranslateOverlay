...
public class ScoreboardTranslator {
    ...
    public static void translate(MatrixStack matrices, ScoreboardObjective objective){
        ...
        scoreboard.getAllPlayerScores(objective).stream()
            ...
            Text rawName = Text.of(entry.getPlayerName());
            Text name = Team.decorateName(team, rawName);
            Text point = Text.of(Integer.toString(entry.getScore()));
        ...
        if (translatedTitle != null && translated != null && !translated.isEmpty()){
            renderScoreboardSidebar(matrices);
        }
    }

    private static void renderScoreboardSidebar(MatrixStack matrices) {
        // 计算最大宽度
        ...
        // 渲染计分板
        renderSidebar(matrices, translated, translatedTitle, maxWidth, originalWidth);
    }
    ...
    private static void renderSidebar(MatrixStack matrices, List<TranslatedSidebarEntry> translated, Text displayName, int maxWidth, int originalWidth) {
        TextRenderer textRenderer = MinecraftClient.getInstance().textRenderer;
        int entryCount = translated.size();
        int totalHeight = entryCount * 9;
        int centerY = RenderUtil.getScaledHeight() / 2 + totalHeight / 3;
        int originalBoxWidth = originalWidth + 4;
        int startX = RenderUtil.getScaledWidth() - maxWidth - 3 - originalBoxWidth;
        int endX = RenderUtil.getScaledWidth() - 1 - originalBoxWidth;
        ...
        // 绘制背景
        DrawableHelper.fill(matrices, startX - 2, startY - 10, endX, startY, 0x33ff99 | backgroundColor2);
        DrawableHelper.fill(matrices, startX - 2, startY, endX, centerY, 0x33ff99 | backgroundColor1);

        // 绘制标题
        int titleX = startX + (maxWidth - textRenderer.getWidth(displayName)) / 2;
        textRenderer.draw(displayName.asOrderedText(), titleX, startY - 9, -1, matrices.peek().getPositionMatrix(), false);

        // 绘制条目
        for (int i = 0; i < entryCount; i++) {
            TranslatedSidebarEntry entry = translated.get(i);
            int entryY = centerY - (entryCount - i) * 9;
            textRenderer.draw(entry.name().asOrderedText(), startX, entryY, -1,  matrices.peek().getPositionMatrix(), false);
        }
    }
