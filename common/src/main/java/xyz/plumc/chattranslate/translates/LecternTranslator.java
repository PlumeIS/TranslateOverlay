    ...
    public static void translate(BlockHitResult rayTraceResult){
        if (rayTraceResult.getType()== HitResult.Type.BLOCK){
            ...
            if (blockEntity instanceof LecternBlockEntity){
                ...
                PlayerInteractBlockC2SPacket packet = new PlayerInteractBlockC2SPacket(Hand.MAIN_HAND, rayTraceResult, minecraft.world.getPendingUpdateManager().getSequence());
                minecraft.getNetworkHandler().sendPacket(packet);
                TickUtil.tickWait(new BookViewScreenPredicate(), ()->{
                    ChatTranslate.logger.info("Waiting a lectern translation with %s in x:%s y:%s z:%s".formatted(Config.getTranslator().getTranslatorName(),
