package xyz.plumc.chattranslate.components;
...
import xyz.plumc.chattranslate.components.animation.animas.SemFunction;
import xyz.plumc.chattranslate.translates.chat.ChatStatus;
import xyz.plumc.chattranslate.translates.chat.TranslatedGuiMessage;
...
public class TranslatedChatComponent {
    ...
    public void render(MatrixStack matrices, int tickCount, int mouseX, int mouseY) {
        ...
        for (int i = 0; i + this.chatScrollbarPos < this.trimmedMessages.size() && i < linesPerPage; ++i) {
            ...
            matrices.translate(0.0f, 0.0f, 50.0f);
            DrawableHelper.drawTextWithShadow(matrices, this.minecraft.textRenderer, line.content(), animaX, lineEndX - animationHeight, 0xFFFFFF + (lineTextOpacity << 24));
            matrices.pop();
        }
        ...
        if (counter >= 1 || isInChat){
            ...
            if (!(resultLineOpacity <= 3) || isInChat){
                ...
                DrawableHelper.drawTextWithShadow(matrices, this.minecraft.textRenderer, Text.translatable("message.chattranslate.result_notice"), 0, lineX+lineSpaced-animationHeight, 0xFFFFFF + (resultLineOpacity << 24));
                matrices.pop();
            }
        }
        ...
        if (listenerSize > 0L) {
            ...
            matrices.push();
            matrices.translate(0.0f, flooredHeight, 0.0f);
            DrawableHelper.fill(matrices, -2, 0, ceiledWidth + 4, 9, appearance << 24);
            matrices.translate(0.0f, 0.0f, 0.0f);
            DrawableHelper.drawTextWithShadow(matrices, this.minecraft.textRenderer, Text.translatable("chat.queue", listenerSize), 0, 1, 0xFFFFFF + (chatOpacity << 24));
            matrices.pop();
        }
        if (isInChat) {
            ...
            if (appearance != perPageHeight) {
                ...
                lineX = ceiledWidth + 4;
                DrawableHelper.fill(matrices, lineX, -af, lineX + 2, -af - lineTextOpacity, 100, boxColor + (lineBackgroundOpacity << 24));
                DrawableHelper.fill(matrices, lineX + 2, -af, lineX + 1, -af - lineTextOpacity, 100, 0xCCCCCC + (lineBackgroundOpacity << 24));
            }
        }
        matrices.pop();
    }
