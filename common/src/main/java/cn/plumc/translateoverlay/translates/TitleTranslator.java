...
public class TitleTranslator {
    ...
    private static void renderActionbar(DrawContext context){
        ...
        if (overlayMessage != null) {
            ...
            int alpha = ColorHelper.Argb.withAlpha((int) (overlayMessageAnimation.step() * 255), -1);
            if (overlayMessageAnimation.isEnd()){
                ...
                if (overlayMessageKeepAnimation.isEnd()){
                    ...
                    alpha = ColorHelper.Argb.withAlpha((int) (Math.min(overlayMessageEndAnimation.step(), 1) * 255), -1);
            ...
            context.drawTextWithBackground(textRenderer, overlayMessage, -width / 2, -4, width, alpha);
    ...
    private static void renderTitle(DrawContext context){
        ...
        if (title != null) {
            ...
            int alpha = ColorHelper.Argb.withAlpha((int) (titleAnimation.step() * 255), -1);
            if (titleAnimation.isEnd()){
                ...
                if (titleKeepAnimation.isEnd()){
                    ...
                    alpha = ColorHelper.Argb.withAlpha((int) (Math.min(titleEndAnimation.step(), 1) * 255), -1);
            ...
            context.drawTextWithBackground(textRenderer, title, -width / 2, -10, width, alpha);
            context.getMatrices().pop();
            if (subtitle != null) {
                context.getMatrices().push();
                context.getMatrices().scale(1.2F, 1.2F, 1.2F);
                int l = textRenderer.getWidth(subtitle);
                context.drawTextWithBackground(textRenderer, subtitle, -l / 2, 13, l, alpha);
                context.getMatrices().pop();
            }
            context.getMatrices().pop();
