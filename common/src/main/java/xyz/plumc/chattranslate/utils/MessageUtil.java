...
public class MessageUtil {
    public static String serializeMutableComponent(Text rawComponent, @Nullable String holder) {
        return serializeMutableComponent(rawComponent.getWithStyle(Style.EMPTY), holder);
    }
    ...
    public static MutableText parseComponent(String message, ChatColor defaultColor) {
        ...
        LiteralText component = (LiteralText) LiteralText.EMPTY.copy();
            ...
            if (c == 167) {
                ...
                if (format != null) {
                    if (format == ChatColor.BOLD) {
                        ...
                        component.setStyle(component.getStyle().obfuscated(true));
                    } else {
                        if (format == ChatColor.RESET) {
                            format = defaultColor;
                        }

                        component.string = builder.toString();
                        components.append(component);
                        builder = new StringBuilder();

                        component = (LiteralText) LiteralText.EMPTY.copy();
