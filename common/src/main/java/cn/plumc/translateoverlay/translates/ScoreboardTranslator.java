...
public class ScoreboardTranslator {
    ...
    private static void renderSidebar(MatrixStack matrices, List<TranslatedSidebarEntry> translated, Text displayName, int maxWidth, int originalWidth) {
        ...
        textRenderer.draw(displayName.asOrderedText(), titleX, startY - 9, -1, matrices.peek().getModel(), false);

        // 绘制条目
        for (int i = 0; i < entryCount; i++) {
            TranslatedSidebarEntry entry = translated.get(i);
            int entryY = centerY - (entryCount - i) * 9;
            textRenderer.draw(entry.name().asOrderedText(), startX, entryY, -1,  matrices.peek().getModel(), false);
        }
    }
