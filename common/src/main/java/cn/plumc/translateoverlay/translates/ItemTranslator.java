...
public class ItemTranslator {
    ...
    public static void translate(ItemStack itemStack, DrawContext context, List<Text> texts, int x, int y) {
        ...
        if ((item == null || !item.equals(itemStack)) && System.currentTimeMillis() - submittedStamp > COOLDOWN_TIME){
            ...
            if (x + originalWidth + translatedWidth + 10 >= context.getScaledWindowWidth()){
                if (y - originalHeight - translatedHeight - 10 <= 0){
                    context.drawTooltip(textRenderer, translated, x, y + originalHeight + 10);
                } else {
                    context.drawTooltip(textRenderer, translated, x, y - translatedHeight - 10);
                }
            } else {
                context.drawTooltip(textRenderer, translated, x + originalWidth + 10, y);
            }
        }
    }
