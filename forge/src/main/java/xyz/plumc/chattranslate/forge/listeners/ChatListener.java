package xyz.plumc.chattranslate.forge.listeners;
...
public class ChatListener {
    ...
    public static void onClientChat(ClientChatEvent event) throws CommandSyntaxException {
        if (event.getMessage().startsWith("!")){
            ...
            ParseResults<ServerCommandSource> parse = ChatTranslateForge.DISPATCHER.parse(event.getMessage().substring(1), source);
            if(!parse.getContext().getNodes().isEmpty()){
                event.setCanceled(true);
                ChatTranslateForge.DISPATCHER.execute(parse);
            }
        }
    }
}
