    ...
    public void render(MatrixStack matrices, int tickCount, int mouseX, int mouseY) {
        ...
        if (counter >= 1 || isInChat){
            ...
            if (!(resultLineOpacity <= 3) || isInChat){
                ...
                DrawableHelper.drawTextWithShadow(matrices, this.minecraft.textRenderer, Text.translatable("message.translateoverlay.result_notice"), 0, lineX+lineSpaced-animationHeight, 0xFFFFFF + (resultLineOpacity << 24));
                matrices.pop();
            }
        }
