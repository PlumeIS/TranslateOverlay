...
public class TranslatedChatComponent {
    private static final Logger LOGGER = LoggerFactory.getLogger(ChatTranslate.MOD_ID);
    ...
    public void render(MatrixStack matrices, int tickCount, int mouseX, int mouseY) {
        ...
        if (counter >= 1 || isInChat){
            ...
            if (!(resultLineOpacity <= 3) || isInChat){
                ...
                DrawableHelper.drawTextWithShadow(matrices, this.minecraft.textRenderer, new TranslatableText("message.chattranslate.result_notice"), 0, lineX+lineSpaced-animationHeight, 0xFFFFFF + (resultLineOpacity << 24));
                matrices.pop();
            }
        }
