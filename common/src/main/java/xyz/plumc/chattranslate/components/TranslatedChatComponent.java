    ...
    public void render(DrawContext guiGraphics, int tickCount, int mouseX, int mouseY) {
        ...
        if (counter >= 1 || isInChat){
            ...
            if (!(resultLineOpacity <= 3) || isInChat){
                ...
                guiGraphics.drawTextWithShadow(this.minecraft.textRenderer, Text.translatable("message.chattranslate.result_notice"), 0, lineX+lineSpaced-animationHeight, 0xFFFFFF + (resultLineOpacity << 24));
                guiGraphics.getMatrices().pop();
            }
        }
